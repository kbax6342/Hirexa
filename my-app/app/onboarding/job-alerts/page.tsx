...
export default function JobAlertsOnboardingPage() {
  ...
  const titlesText = useMemo(() => {
    ...
    // limit how many we display to keep it pretty
    const max = 5;
  ...
  async function onNext() {
    ...
    try {
      ...
      if (!res.ok) {
        setSaveError(data?.error ?? "Failed to save email");
        setSaving(false);
        return;
      }
    ...
    <main className="min-h-[calc(100vh-64px)] bg-white">
      <div className="mx-auto w-full max-w-5xl px-6 py-10">
        ...
        <div className="mt-16 flex items-center justify-between">
          ...
          <Link
            href="/onboarding/choose-workplace"
            aria-disabled={!emailLooksValid}
            className={[
            ...
            onClick={(e) => {
              if (!emailLooksValid) e.preventDefault();
            }}
          >
            Next
          </Link>
