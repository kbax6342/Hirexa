...
function normalizeExperience(raw: any, idx: number): Required<Pick<Experience, "title" | "company">> &
  Omit<Experience, "title" | "company"> & { id: string; bullets: string[] } {
  return {
...
export default function Step2ResumeClient() {
  ...
  const resumeId = params.get("resumeId"); // ✅ from Step 1
  ...
  useEffect(() => {
    // get resumeId from the URL (client-only)
    const rid = new URLSearchParams(window.location.search).get("resumeId");
  
    if (!rid) {
      setError("Missing resumeId. Please go back and upload your resume again.");
      setExperiences([]);
      return;
    }
    ...
    (async () => {
      ...
      try {
        const res = await fetch(`/api/resume/experience?resumeId=${encodeURIComponent(rid)}`, {
          cache: "no-store",
        });
        ...
        const normalized = raw.map((r, idx) => normalizeExperience(r, idx));
    ...
    <div className="min-h-screen bg-white">
      <main className="mx-auto max-w-5xl px-6 pb-16">
        ...
        {!loading && !hasResults && !error && (
          <p className="mt-6 text-sm text-gray-600">
            No work experience found yet. If this seems wrong, go back and re-upload your resume.
          </p>
        )}
        ...
        <div className="mt-6 space-y-5">
          {experiences.map((exp, i) => {
            ...
            // ✅ bullet-safe (ALWAYS an array)
            const bullets = Array.isArray(exp.bullets) ? exp.bullets : [];
            const bulletsToShow = isOpen ? bullets : bullets.slice(0, 3);
                ...
                <div className="flex items-start gap-4 px-5 py-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3 text-blue-700">
                        <button type="button" className="rounded-md p-2 hover:bg-blue-50" title="Edit (coming soon)">
                          <PencilIcon />
                        </button>
                        <button type="button" className="rounded-md p-2 hover:bg-blue-50" title="Delete (coming soon)">
                          <TrashIcon />
                        </button>
                      </div>
                    </div>
        ...
        <div className="mt-10 flex items-center justify-between">
          <Link href="/questions/step2" className="rounded-full border px-6 py-3 text-sm font-semibold">
            Back
          </Link>

          <Link href="/questions/step3" className="rounded-full bg-blue-600 px-6 py-3 text-sm font-semibold text-white">
            Next
          </Link>
        </div>
      </main>
