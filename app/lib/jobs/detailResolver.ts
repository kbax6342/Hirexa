...
import type { Job, JobDetail, JobPretty, JobSource } from "./types";
...
async function fetchUSAJobsDetail(id: string, summary?: Job | null) {
  ...
  try {
    ...
    const duties = details?.MajorDuties?.map((item) => cleanText(item)).filter(Boolean) ?? [];
    const requirements = [details?.QualificationSummary, details?.WhoMayApply]
    ...
    return finalizeDetail({
      ...
      employmentType: schedule.join(", ") || null,
      ...
      description:
        cleanPlainText(details?.JobSummary) || summary?.description,
      descriptionPlain: cleanPlainText(details?.JobSummary) || null,
      requirements,
      duties,
      metadata: {
      ...
      detailLevel: duties.length > 0 || requirements.length > 0 ? "full" : "partial",
      providerHasFullDetails: duties.length > 0 || requirements.length > 0,
    }, {
      fullDetailsUnavailable: duties.length === 0 && requirements.length === 0,
    });
  } catch {
